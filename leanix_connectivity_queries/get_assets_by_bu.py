...
associated with a specific Business Unit / Business Capability.
"""
...
def filter_factsheets_by_business_unit(factsheets_data, bu_name, exact_match=False):
    """Filter factsheets associated with a specific business unit/capability."""
    filtered_items = []
    ...
    for edge in factsheets_data['data']['allFactSheets']['edges']:
        ...
        for bc in business_capabilities:
            ...
            if exact_match:
                if bc_name == bu_name_lower or bc_display == bu_name_lower:
                    matched = True
                    break
            else:
                if bu_name_lower in bc_name or bu_name_lower in bc_display:
                    matched = True
                    break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Get all LeanIX assets for a specific Business Unit / Business Capability',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Get all Hobbs Brook assets (partial match)
  python get_assets_by_bu.py "Hobbs Brook"
  
  # Get exact match only
  python get_assets_by_bu.py "Hobbs Brook" --exact
  
  # Specify output file
  python get_assets_by_bu.py "Engineering" --output engineering_assets.json
        """
    )
    
    parser.add_argument('business_unit', 
                        help='Business Unit or Business Capability name to filter by')
    parser.add_argument('--exact', action='store_true',
                        help='Require exact match (case-insensitive)')
    parser.add_argument('--output', '-o', 
                        help='Output JSON filename (default: <bu_name>_assets.json)')
    
    args = parser.parse_args()
    
    bu_name = args.business_unit
    exact_match = args.exact
    
    # Generate output filename from BU name if not specified
    if args.output:
        output_file = args.output
    else:
        safe_name = bu_name.lower().replace(' ', '_').replace('/', '_')
    ...
    try:
        ...
        # Filter for business unit
        bu_assets = filter_factsheets_by_business_unit(all_factsheets_data, bu_name, exact_match)
        
        match_type = "exact match" if exact_match else "partial match"
        print(f"\n{'='*70}")
        print(f"ASSETS FOR BUSINESS UNIT: {bu_name} ({match_type})")
        print(f"{'='*70}")
        print(f"\nFound {len(bu_assets)} asset(s)\n")
        ...
        if bu_assets:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(bu_assets, f, indent=2, ensure_ascii=False)
            print(f"\nFull data saved to: {output_file}")
        else:
            print(f"No assets found for business unit: {bu_name}")
